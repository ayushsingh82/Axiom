...
export default function Home() {
  return (
    <main className="min-h-screen bg-black text-white font-[family-name:var(--font-syne)] relative overflow-hidden">
      ...
      {/* Vertical rail lines — outer decoration */}
      <div className="pointer-events-none fixed inset-0 z-0" aria-hidden>

        {/* ══ LEFT RAIL ══ */}
        {/* top segment — bright white */}
        ...
        {/* bottom segment — silver with scan */}
        ...
        {/* + crosshair — horizontal arm extends LEFT only, white */}
        ...
        {/* LEFT outer decorations — to the LEFT of the rail */}
        {/* small tick marks on the outer left */}
        ...
        {/* small animated dot sliding down the outer edge */}
        ...
        {/* + crosshair — horizontal arm extends RIGHT only, white */}
        ...
        {/* RIGHT outer decorations — to the RIGHT of the rail */}
        {[200, 320, 460, 600, 760].map((top, i) => (
      ...
      <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12 items-center">
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <Link
                  href="/oracle"
                  className="px-7 py-3 bg-white text-black text-sm font-semibold hover:bg-white/90 active:scale-95 transition-all"
                >
                  Try the Oracle
                </Link>
                <Link
                  href="/docs"
                  className="px-7 py-3 border border-white/20 text-white/70 text-sm hover:border-[#3B82F6]/50 hover:text-[#3B82F6] transition-all"
                >
                  Read the Docs
                </Link>
              </div>
            ...
            <div className="hidden lg:flex items-center justify-center">
              <div className="relative w-72 h-72">
                ...
                {/* Floating badges */}
                <div className="absolute -top-4 -right-2 text-[10px] font-mono px-2 py-1 border border-[#3B82F6]/30 text-[#3B82F6] bg-black/80" style={{ animation: "fade-in-up 0.6s ease-out forwards" }}>
                  x402
                </div>
                <div className="absolute -bottom-4 -left-2 text-[10px] font-mono px-2 py-1 border border-[#8B5CF6]/30 text-[#8B5CF6] bg-black/80" style={{ animation: "fade-in-up 0.8s ease-out forwards" }}>
                  ERC-7710
                </div>
                <div className="absolute top-1/2 -right-8 text-[10px] font-mono px-2 py-1 border border-[#06B6D4]/30 text-[#06B6D4] bg-black/80 -translate-y-1/2" style={{ animation: "fade-in-up 1s ease-out forwards" }}>
                  1Shot
                </div>
              </div>
            </div>
        ...
        <section className="px-8 py-16 max-w-5xl mx-auto">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-white/10 border border-white/10 overflow-hidden">
            {[
              ...
              <div key={stat.label} className="bg-black px-6 py-7 group hover:bg-white/[0.02] transition-colors">
                <div className="flex items-baseline gap-1 mb-1">
                  <span className="text-3xl font-bold text-white">{stat.value}</span>
                  <span className="text-sm text-[#3B82F6] font-mono">{stat.unit}</span>
                </div>
        ...
        <section className="px-8 py-24 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-px bg-white/10 border border-white/10 overflow-hidden">
            {[
              ...
              <div key={item.step} className="bg-black px-6 py-8 hover:bg-white/[0.02] transition-colors group relative overflow-hidden">
        ...
        <section className="px-8 py-24 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-white/10 border border-white/10 overflow-hidden">
            {[
              ...
              <div key={card.title} className="bg-black px-6 py-8 hover:bg-white/[0.02] transition-colors group relative overflow-hidden">
        ...
        <section className="px-8 py-24 max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-white/10 border border-white/10 overflow-hidden">
            {[
              ...
              <div key={item.name} className="bg-black px-6 py-8 hover:bg-white/[0.02] transition-colors group relative overflow-hidden">
                <div className="absolute top-0 left-0 right-0 h-[2px] opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: item.color }} />
                <div className="flex items-center justify-between mb-4">
                  <span className="text-white font-semibold text-sm">{item.name}</span>
                  <span className="text-[10px] border px-2 py-0.5 font-mono" style={{ color: item.color, borderColor: `${item.color}30` }}>{item.tag}</span>
                </div>
                <p className="text-sm text-white/40 leading-relaxed">{item.desc}</p>
              </div>
            ))}
        ...
        <section className="px-8 pb-24 max-w-5xl mx-auto">
          <div className="border border-white/10 bg-white/[0.02] px-10 py-12 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#3B82F6] via-[#8B5CF6] to-[#06B6D4]" />
            <div>
              <h3 className="text-xl font-bold text-white mb-2">Start inferring in 60 seconds</h3>
            ...
            <Link
              href="/oracle"
              className="flex-shrink-0 px-7 py-3 bg-white text-black text-sm font-semibold hover:bg-white/90 active:scale-95 transition-all"
            >
              Open the Oracle →
            </Link>
          </div>
        ...
        {/* Footer */}
        <footer className="border-t border-white/10 py-8 flex items-center justify-between text-xs text-white" style={{ paddingLeft: "calc(12% + 2rem)", paddingRight: "calc(12% + 2rem)" }}>
